...
module.exports = blasterjs = Class(
  /** @lends Biojs.blasterjs# */
  {
    ...
    constructor: function (options) {
      ...
      self.onSelectCreated = this.opt.onSelectCreated;
    ...
    _displayAlignments: function (evt, self) {
      ...
      function BlastAlignment(accession,
                              taxon,
                              length,
                              totalScore,
                              queryCover,
      ...
      function parseBlastXML(content) {
        ...
        for (var i = 0; i < lines.length; i++) {
          if (lines[i].startsWith('<Hit>')) {
            ...
            const accession = idSplit[1];
            let taxon = capitalizeWords(idSplit[2].replace("_", " "));
            ...
            var alignment = new BlastAlignment(accession, taxon, length, totalScore.toFixed(1), getQueryCover(hsps_a, getQueryLength(content)), hsps_a);
      ...
      function createQueryDiv(colored) {
        ...
        divSpace.style.flex = '0 0 auto';
        divSpace.innerHTML = 'QUERY';
        divSpace.style.fontWeight = '900';
        divQuery.style.flex = '1 1 auto';
      ...
      function createNumbersDiv(lenght) {
        ...
        divNumbers.style.flex = '1 1 auto';
      ...
      function divideDivNumbers(container, lenght) {
        container = createDivisionsDivNumbers(container, lenght);
        return container;
      }

      function createDivisionsDivNumbers(container, length) {
        const div1 = document.createElement('div');
        const div2 = document.createElement('div');
        div2.style.textAlign = 'right';
        div2.style.flex = '1 1 auto';
        div2.style.minWidth = '0';
        div2.innerHTML = `${length}`;
        div1.style.flex = '1 1 auto';
        div1.style.minWidth = '0';
        div1.innerHTML = `${0}`;
      ...
      function createAlignmentDiv(color, alignment, queryLength) {
        ...
        const container = document.createElement('div');
        const initSpace = document.createElement('div');
        const alignmentContainer = document.createElement('div');
        alignmentContainer.style.flex = '1 1 auto';
        alignmentContainer.style.minWidth = '0';
        alignmentContainer.style.display = 'flex';
        initSpace.style.width = '50px';
        container.style.minHeight = '12px';
        container.style.display = 'flex';
        container.appendChild(initSpace);
        container.appendChild(alignmentContainer);
        ...
        for (var i = 0; i < noOver.length; i++) {
          ...
          let width = 0;
          let widthWhite;
          width = (noOver[i].end - (noOver[i].start - 1)) * 100 / queryLength;
          if (i == 0) {
            widthWhite = (noOver[i].start - 1) * 100 / queryLength;
          } else {
            widthWhite = (noOver[i].start - noOver[i - 1].end) * 100 / queryLength;
          }
          white.style.width = `${widthWhite}%`;
          link.style.width = `${width}%`;
          colored.style.width = '100%';
          ...
          alignmentContainer.appendChild(white);
          alignmentContainer.appendChild(link);
        }
        return container;
      }
      ...
      function createChangeScoringButton(queries, hsps, position, alignments, lenght, colored, scoring, self) {
        ...
        button.className = 'btn-blasterjs';
      ...
      function downloadAlignmentsImg(type) {
        const container = document.getElementById("blast-multiple-alignments-container");
        html2canvas(container).then(canvas => {
          var link = document.createElement('a');
          link.download = `multiple-alignments.${type.toLowerCase()}`;
      ...
      function createSelectQueries(queries, hsps, position, queryLength, colored, scoring, self) {
        ...
        self.customGoToQuery = (value) => {
          goToQuery(queries, hsps, value, queryLength, colored, scoring, self);
        };
        self.onSelectCreated ? self.onSelectCreated() : null;
        return selectQuery;
      }
      ...
      function createAlignmentComparison(queries, hsps, position, alignments, queryLength, colored, scoring, self) {
        ...
        input.name = 'defline';
        input.type = 'text';
        input.value = ' Mouse over to show defline and scores';
        input.style.width = '556px';
        input.style.padding = '1px';
        input.style.border = 0;
        input.style.cursos = 'auto';
        input.style.marginLeft = "50px";
        container.id = 'blast-multiple-alignments-container';
        container.style.padding = '10px';
        container.style.width = '100%';
        container.style.backgroundColor = '#ffffff';
        if (queries.length > 1) {
          const resultForContainer = document.createElement('div');
          ...
          resultForContainer.style.display = 'none';
          resultsFor.innerHTML = 'RESULTS FOR:';
          container.appendChild(resultForContainer);
          resultForContainer.appendChild(resultsFor);
          resultForContainer.appendChild(selectQuery);
        }
        container = createHeader(container, colored, scoring, queryLength);
        ...
        containerButtons.style.flexWrap = 'wrap';
        ...
        blastDiv.appendChild(container);
        blastDiv.appendChild(input);
        blastDiv.appendChild(containerButtons);
      }
      ...
      function createTableHeader() {
        ...
        const theadAccession = document.createElement('th');
        const theadTaxon = document.createElement('th');
        const theadScore = document.createElement('th');
        const theadEval = document.createElement('th');
        const theadIdent = document.createElement('th');
        const theadPos = document.createElement('th');
        const theadGaps = document.createElement('th');
        table.className = "table table-striped aligment-table-blaster-js";
        theadAccession.innerHTML = 'Accession'.bold();
        theadTaxon.innerHTML = 'Taxon'.bold();
        ...
        theadTr.appendChild(theadAccession);
        theadTr.appendChild(theadTaxon);
      ...
      function createTableButtons(alignments) {
        ...
        const butCSV = document.createElement('button');
        const butImgJPEG = document.createElement('button');
        const butImgPNG = document.createElement('button');
        container.style.display = 'flex';
        container.style.flexDirection = 'row-reverse';
        container.id = "blast-alignments-table-buttons";
        containerButton.style.display = 'flex';
        containerButton.style.flexWrap = 'wrap';
        containerButton.style.backgroundColor = '#ffffff';
        containerButton.style.padding = '1rem 1rem';
        containerButton.style.borderTopLeftRadius = '30px';
        containerButton.style.borderTopRightRadius = '30px';
        butCSV.style.marginRight = '10px';
        butCSV.className = 'btn-blasterjs';
        butCSV.innerHTML = 'Download as CSV';
        butCSV.onclick = function () {
          downloadTableCSV(alignments);
        };
        butImgPNG.className = 'btn-blasterjs';
        butImgPNG.innerHTML = 'Download as PNG';
        butImgPNG.onclick = function () {
          downloadTableImg('PNG');
        };
        butImgPNG.style.marginRight = '10px';
        butImgJPEG.className = 'btn-blasterjs';
        butImgJPEG.innerHTML = 'Download as JPEG';
        butImgJPEG.onclick = function () {
          downloadTableImg('JPEG');
        };
        container.appendChild(containerButton);
        containerButton.appendChild(butCSV);
        containerButton.appendChild(butImgPNG);
        containerButton.appendChild(butImgJPEG);
        return container;
      }
      ...
      function createAlignmentTable(alignments, self) {
        ...
        for (var i = 0; i < alignments.length; i++) {
          ...
          const tdAccesion = document.createElement('td');
          tdAccesion.innerHTML = alignments[i].accession;
          const tdTaxon = document.createElement('td');
          tdTaxon.innerHTML = alignments[i].taxon;
          ...
          tr.appendChild(tdAccesion);
          tr.appendChild(tdTaxon);
      ...
      function downloadTableImg(type) {
        ...
        html2canvas(container).then(canvas => {
          var link = document.createElement('a');
          link.download = `table-alignments.${type.toLowerCase()}`;
          link.href = canvas.toDataURL(`image/${type.toLowerCase()}`);
          link.click();
        });
      }
      ...
      function createSingleAlignment(alignment, self, hsps, hsp) {
        ...
        const spaceInMiddleFlex = document.createElement('div');
        const buttonsContainer = document.createElement('div');
        const statsDiv = document.createElement('div');
        const scoreDiv = document.createElement('div');
        const expectedDiv = document.createElement('div');
        const identitiesDiv = document.createElement('div');
        const positivesDiv = document.createElement('div');
        ...
        alignmentPre.style.paddingBottom = '110px';
        ...
        alignmentDiv.style.backgroundColor = '#ecf0f1';
        alignmentPre.style.overflow = 'unset';
        alignmentPre.style.marginBottom = '0';
        alignmentContainer.style.margin = '0 auto';
        alignmentContainer.style.display = 'table';
        alignmentContainer.style.paddingTop = '30px';
        alignmentDiv.style.overflow = 'auto';
        alignmentDiv.style.textAlign = 'right';
        buttonDownloadPNG.className = 'btn-blasterjs';
        ...
        buttonDownloadJPEG.className = 'btn-blasterjs';
        ...
        span.innerHTML = alignment.accession;
        span.style.fontWeight = 'bold';
        span.style.fontSize = '15px';
        span.style.fontFamily = 'Helvetica,Arial,sans-serif';
        spaceInMiddleFlex.style.flex = '1 1 auto';
        spaceInMiddleFlex.style.minWidth = '0';
        buttonsContainer.style.display = 'flex';
        buttonsContainer.style.paddingRight = '1rem';
        buttonsContainer.style.columnGap = '0.5rem';
        buttonsContainer.style.position = 'absolute';
        buttonsContainer.style.right = '0px';
        buttonsContainer.style.paddingTop = '55px';
        buttonsContainer.style.flexDirection = 'row-reverse';
        buttonsContainer.id = "blast-single-alignment-buttons";
        ...
        alignmentPre.appendChild(titleContainer);
        titleContainer.appendChild(span);
        titleContainer.appendChild(spaceInMiddleFlex);
        buttonsContainer.appendChild(buttonDownloadJPEG);
        buttonsContainer.appendChild(buttonDownloadPNG);
        ...
        if (hsps > 1) {
          ...
          buttonNext.style.marginTop = '10px';
          buttonNext.style.backgroundColor = '#efefef';
          buttonNext.style.border = '1px solid #d9d9d9';
          buttonNext.style.right = '3px';
          if (hsp == hsps - 1)
            var goTo = 0;
          else
            var goTo = hsp + 1;
          ...
          alignmentPre.appendChild(buttonsContainer);
          alignmentPre.appendChild(buttonNext);
        }
      }

      function downloadSingleAlignmentImg(alignment, type) {
        const container = document.getElementById("blast-single-alignment");
        const buttons = document.getElementById("blast-single-alignment-buttons");
        const nextButton = document.getElementById('blast-single-alignment-next');
        buttons.style.display = "none";
        nextButton.style.display = "none";
        html2canvas(container).then(canvas => {
          canvas.style.width = `${container.offsetWidth}px`;
          var link = document.createElement('a');
          link.download = `table-blast-single.${type.toLowerCase()}`;
          link.href = canvas.toDataURL(`image/${type.toLowerCase()}`);
          link.click();
